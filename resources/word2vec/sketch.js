...
function setup() {
  ...
  //let betweenResults = select("#results4");
  ...
  let vectorInput = select("#vectorInput");
  let vectorButton = select("#submit4");
  let vectorResults = select("#results4");
  ...
  vectorButton.mousePressed(() => {
    let vector = word2Vec.model[vectorInput.value()].dataSync();
    vectorResults.html(JSON.stringify(vector,null,2));
  });
}
